...
def analyze_and_chart(csv_path, session_id):
    try:
        ...
        ax1 = axes[0, 0]
        ax1.bar(range(len(reason_summary)), reason_summary['total_minutes'], color='#2563eb', alpha=0.8)
        ...
        ax2.bar(shift_summary.index, shift_summary.values, color=colors_shift[:len(shift_summary)], alpha=0.85, edgecolor='white', linewidth=1.5)
        ...
        ax3.barh(area_summary.index, area_summary.values, color=colors_area, alpha=0.85, edgecolor='white', linewidth=1.5)
        ax3.set_xlabel('Total Downtime (minutes)')
        ax3.set_title('Downtime by Area/Zone', fontweight='bold', pad=10)
        ax3.grid(axis='x', alpha=0.3)
        ax4 = axes[1, 1]
        ax4.bar(range(len(equip_summary)), equip_summary.values, color='#7c3aed', alpha=0.85, edgecolor='white', linewidth=1.5)
